...
**ContactForm's HubSpot branch is the supported default (2026-08-14), not the fallback.** Every Lock 8 client stays in the HubSpot ecosystem for form management unless a specific client is noted otherwise — always fill in `hubspot_portal_id`/`hubspot_form_id` on a client build. The plain Netlify Forms path (`data-netlify="true"`, no HubSpot IDs set) is the fallback for a non-HubSpot client, and used as the real path for `primer-2026`. It used to be fragile under SSR — Netlify's form-detection only scans prerendered static HTML at build time, which an SSR-only site (`output: 'server'`, see "Rendering mode" below) never produces — **fixed 2026-08-20** via `public/__forms.html`, a static stub form Netlify's build actually scans, kept in field-name sync with `ContactForm.astro`'s real form by hand (see that file's top comment). Submission notifications (e.g. emailing a specific address) are a Netlify dashboard setting (Forms → notifications) with this path, not a per-block Storyblok field — a true per-block "notify this email" field would mean building Option B (a third-party transactional email API integration) instead, parked until a client actually needs it.
