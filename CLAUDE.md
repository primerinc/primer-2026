...
`Gallery` (2026-08-17) is headline/paragraph + a grid of images, added to `page`/`resource`/`campaign_page`'s body whitelists. Two layout modes: `masonry` (default — natural aspect ratio per image, packed via CSS multi-column so tile heights vary, Pinterest-style) and `grid` (every tile forced to a uniform 1:1 square via `object-fit: cover`). One build note: don't append `/smart` to a Storyblok image transform URL — it's not a real modifier in this codebase's convention (confirmed broken in testing) and breaks the image entirely; every fixed-crop image here (Team's 400×400, avatars) just uses `/m/{w}x{h}/filters:quality(85)` and lets Storyblok's default center-crop handle it.

Lightbox (2026-08-20): `lightbox_enabled` Boolean, default false — the original "no lightbox by design" scope call from launch was explicitly a "not yet," not permanent; this is that follow-up. Off by default so every existing Gallery instance is unaffected; per-instance opt-in via the field, not a site-wide setting. Vanilla JS (no framework, matches the rest of this library), scoped per-component-instance via the blok's `_uid` so multiple lightbox Galleries on one page don't collide. Full-screen modal, prev/next nav, Escape/click-outside/arrow-key support, focus trap while open, focus restored to the clicked tile on close. Deliberately not gated behind the `!inVisualEditor` check that disables scroll-reveal animations in BaseLayout — that gate exists so editors aren't stuck scrolling past invisible content while editing fields, which doesn't apply here; the lightbox is a real interactive feature editors should be able to test in the Visual Editor too.
...
**ContactForm's HubSpot branch is the supported default (2026-08-14), not the fallback.** Every Lock 8 client stays in the HubSpot ecosystem for form management unless a specific client is noted otherwise — always fill in `hubspot_portal_id`/`hubspot_form_id` on a client build. The fallback for a non-HubSpot client (no HubSpot IDs set) is **Web3Forms** (`WEB3FORMS_ACCESS_KEY` env var — free tier, api.web3forms.com/submit, called directly from client-side JS with no backend needed), and it's the real path for `primer-2026` as of **2026-08-21**. This replaced an earlier Netlify Forms fallback, dropped ahead of this site's Netlify → Cloudflare hosting migration since Netlify Forms is Netlify-proprietary with no Cloudflare equivalent — Web3Forms has no hosting-platform dependency at all, so the swap was unrelated to the hosting move itself and didn't need to wait for it. (The `public/__forms.html` static-stub workaround that Netlify Forms needed under SSR no longer applies and has been removed.) Submission notifications are a Web3Forms dashboard setting, not a per-block Storyblok field — a true per-block "notify this email" field would still mean building Option B (a client-owned transactional email API integration) instead, parked until a client actually needs it.
